...
func_read_z <- function(chunk) {
  las <- readLAS(chunk)
  if (is.null(las) || nrow(las) == 0) {
    return(NULL)
  } # Checks if the LAS chunk is empty or has no points
...
mycsf <- csf(sloop_smooth = FALSE, cloth_resolution = 0.2, iterations = 500, class_threshold = 0.005)
...
ggplot(df, aes(x = Z)) +
  geom_histogram(
    bins = 30, # You may adjust the number of bins for better data representation
